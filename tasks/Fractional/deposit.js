...
task("functions-deposit", "Initiates a request from a Functions client contract to deposit and fractionalize")
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    const vaultContractAddress = env[network.name].testnet.fractionalVaultContractAddress
    ...
    const clientContract = await ethers.getContractAt("VaultFunctionsConsumer", vaultContractAddress)
    const nftContract = await  ethers.getContractAt("FractionalNFT",nftContractAddress);
    ...
    if (!existingConsumers.includes(vaultContractAddress.toLowerCase())) {
      throw Error(`Consumer contract ${vaultContractAddress} is not registered to use subscription ${subscriptionId}`)
    }
    ...
    //deposit request
    const transactionEstimateGas = await clientContract.estimateGas.depositNFT(
      taskArgs.tokenid,
      requestConfig.source,
      requestConfig.secrets && Object.keys(requestConfig.secrets).length > 0 ? simulatedSecretsURLBytes : [],
      requestConfig.args ?? [],
    ...
    const spinner = utils.spin({
      text: `Submitting transaction for VaultFunctionsConsumer contract ${vaultContractAddress} on network ${network.name}`,
    })
    ...
    await new Promise(async (resolve, reject) => {
      ...
      try {
        let dynamicArgs = [];
        dynamicArgs.push(String(tokenId))
        // Initiate the on-chain request after all listeners are initialized
        requestTx = await clientContract.depositNFT(
          tokenId,
          request.source,
          request.secrets ?? [],
          request.args ?? dynamicArgs,
          subscriptionId,
          gasLimit,
          overrides
        )
      ...
      spinner.stop()
      requestId = requestTxReceipt.events[3].args.id

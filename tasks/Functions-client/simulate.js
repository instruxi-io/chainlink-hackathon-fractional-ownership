...
task("functions-simulate", "Simulates an end-to-end fulfillment locally for the FunctionsConsumer contract")
  ...
  .addOptionalParam(
    ...
    `${__dirname}/../../Functions-total-reserve-config.js`,
    types.string
  )
  .setAction(async (taskArgs, hre) => {
    ...
    const priceFeedAddress = ethers.constants.AddressZero;
    const clientFactory = await ethers.getContractFactory("VaultFunctionsConsumer")
    const client = await clientFactory.deploy(nftContract.address, tokenContract.address, priceFeedAddress, oracle.address)
    await client.deployTransaction.wait(1)

    //mint, set, deposit, approve token before 
    const mintTxn = await nftContract.mint(taskArgs.tokenid);
    const setVaultTxn = await tokenContract.setVault(client.address);
    const setApprovalTxn = await nftContract.setApprovalForAll(client.address, true);
    ...
    await new Promise(async (resolve) => {
      ...
      const requestId = requestTxReceipt.events[3].args.id
      console.log("Depositor: ",await client.depositor(taskArgs.tokenid));
      ...
      registry.on(
        ...
        async (
          ...
          if (requestId == eventRequestId) {
            ...
            // const fulfillGasUsed = await getGasUsedForFulfillRequest(success, result, oracle)
...
const getGasUsedForFulfillRequest = async (success, result, oracle) => {
  const subscriptionId = createSubscriptionReceipt.events[0].args["subscriptionId"].toNumber()
  ...
  await client.deployTransaction.wait(1)
  //mint, set, deposit, approve token before 
  const mintTxn = await nftContract.mint(1);
  const setVaultTxn = await tokenContract.setVault(client.address);
  const setApprovalTxn = await nftContract.setApprovalForAll(client.address, true);
  ...
  //   gasLimit
  // )
  // console.log("Depositor: ", client.depositor(1));
